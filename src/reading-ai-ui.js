...
function suggestionItem(text, label, action) {
  ...
  item.className = 'item reading-ai-suggestion';
...
async function saveQuestion(resourceId, text) {
  ...
  await api.addRelation({ source_type: 'resource', source_id: resourceId, target_type: 'question', target_id: question.id }, user.id);
}

async function saveConnection(resourceId, connection) {
  const user = await owner();
  await api.addRelation({ source_type: 'resource', source_id: resourceId, target_type: connection.type, target_id: connection.id }, user.id);
}
...
async function renderBaseResult(panel, result, resourceId) {
  ...
  root.append(section('핵심 주장·논리', result.claims.map((text) => suggestionItem(text, '메모로 저장', () => saveNote(resourceId, text, '생각')))));
  root.append(section('읽으며 남길 질문', result.questions.map((text) => suggestionItem(text, '질문으로 저장', () => saveQuestion(resourceId, text)))));
  if (result.connections.length) {
    const labels = await connectionLabels();
    root.append(section('기존 기록과 연결 후보', result.connections.map((connection) => {
      const label = labels.get(`${connection.type}:${connection.id}`) || (connection.type === 'topic' ? '기존 주제' : '기존 질문');
      return suggestionItem(connection.reason ? `${label} — ${connection.reason}` : label, '연결', () => saveConnection(resourceId, connection));
    })));
  }
...
function renderExpansion(panel, result, resourceId) {
  ...
  if (!expansion) return;
  for (const [title, values, noteType] of [['긴장 지점', expansion.tensions, '생각'], ['반론·대안 관점', expansion.counterpoints, '반론'], ['발전시킬 문제의식', expansion.framings, '글감']]) {
    if (!values.length) continue;
    root.append(section(title, values.map((text) => suggestionItem(text, '메모로 저장', () => saveNote(resourceId, text, noteType)))));
  }
}
...
function createPanel(resourceId) {
  ...
  runButton.addEventListener('click', async () => {
    ...
    try { await renderBaseResult(panel, normalizeAiReadResult(await api.analyzeResource(resourceId, 'read')), resourceId); setStatus(panel, '제안이 준비되었습니다. 필요한 항목만 기록으로 저장하세요.'); }
  ...
  expandButton.addEventListener('click', async () => {
    ...
    try { renderExpansion(panel, normalizeAiReadResult(await api.analyzeResource(resourceId, 'expand')), resourceId); setStatus(panel, '생각 확장 제안이 준비되었습니다. 필요한 항목만 저장하세요.'); }
