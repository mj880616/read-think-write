...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const beta = await requireBetaAccess(user);
    ...
    if (beta.role !== 'admin') {
      const { error: quotaError } = await admin.rpc('rtw_consume_ai_quota', { p_owner_id: user.id, p_action: mode, p_limit: 3 });
      if (quotaError) {
        if (quotaError.message.includes('daily_ai_limit_reached')) throw new Error(`오늘의 ${mode === 'expand' ? '생각 확장' : 'AI 읽기'} 무료 이용 횟수(3회)를 모두 사용했습니다.`);
        throw quotaError;
      }
    }
