...
function bookmarkView(){const rm=new Map(state.resources.map(r=>[r.id,r]));const rb=state.bookmarks.filter(b=>b.bookmark_type==='resource');const pb=state.bookmarks.filter(b=>b.bookmark_type==='passage');root.innerHTML=shell(`<section class="hero bookmark-hero"><div class="eyebrow">책갈피</div><h1>다시 볼 곳</h1><p>다시 읽을 자료와 본문에서 표시해 둔 대목을 모아 본다.</p></section><section class="card bookmark-section"><h2>자료</h2><div class="stack">${rb.map(b=>rm.get(b.resource_id)).filter(Boolean).map(resourceItem).join('')||empty('책갈피한 자료가 없음')}</div></section><section class="card bookmark-section"><h2>본문</h2><div class="passage-bookmark-list">${pb.map(b=>{const rr=rm.get(b.resource_id);return `<div class="passage-bookmark-row"><a class="passage-bookmark-main" href="${href('/read/'+b.resource_id+'/?bookmark='+encodeURIComponent(b.id))}" data-passage-bookmark="${b.id}" data-resource-id="${b.resource_id}"><span class="passage-bookmark-source">${esc(rr?.author||rr?.source_name||'')}${(rr?.author||rr?.source_name)?' · ':''}${esc(rr?.title||'자료')}</span><strong class="passage-bookmark-text">“${esc(b.selected_text||'')}”</strong>${b.note?`<span class="passage-bookmark-note">메모 · ${esc(b.note)}</span>`:''}</a><button class="passage-bookmark-delete" data-delete-bookmark="${b.id}" type="button" aria-label="책갈피 삭제" title="삭제">×</button></div>`}).join('')||empty('본문 책갈피가 없음')}</div></section>`,'bookmarks');bindCommon();bindResourceBookmarkButtons();document.querySelectorAll('[data-passage-bookmark]').forEach(a=>a.addEventListener('click',e=>{e.preventDefault();const url=new URL(a.href,location.href);history.pushState({},'',url.pathname+url.search);render();}));document.querySelectorAll('[data-delete-bookmark]').forEach(x=>x.addEventListener('click',async()=>{await api.deleteBookmark(x.dataset.deleteBookmark);await refreshState();bookmarkView();}));}
