...
const data=await response.json();if(!response.ok)throw new Error(data?.error?.message||'웹 추천 요청에 실패했습니다.');const raw=outputText(data).trim();if(!raw)throw new Error('추천 결과가 비어 있습니다. 다시 눌러주세요.');let parsed;try{parsed=JSON.parse(raw);}catch{console.error('Recommendation JSON parse failed',raw.slice(0,300));throw new Error('추천 결과 형식을 완성하지 못했습니다. 다시 눌러주세요.');}const excluded=new Set([...savedUrls,...recentUrls].map(u=>safeUrl(u)).filter(Boolean));const seen=new Set<string>();
