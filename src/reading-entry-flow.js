...
function basePath() { return APP_BASE.replace(/\/$/, ''); }
function appHref(path) { return `${basePath()}${path}`; }
function routePath() { return location.pathname.replace(basePath(), '') || '/'; }
...
function addHomeQuickAdd() {
  ...
  section.innerHTML = `
    <form id="home-quick-add" class="form">
      <div class="field"><label>새 글 바로 추가</label><input name="url" type="url" inputmode="url" placeholder="읽을 글 URL을 붙여넣기" required></div>
      <div class="inline-actions"><button class="btn" type="submit">가져오기</button><a class="btn secondary" href="${appHref('/read/?new=1')}">직접 입력</a></div>
    </form>`;
...
function showNewReadingState() {
  ...
  const eyebrow = hero.querySelector('.eyebrow'); const title = hero.querySelector('h1'); const description = hero.querySelector('p');
...
export function enhanceReadingEntryFlow() {
  ...
  if (newMode) showNewReadingState(); else showReadListState();
}
